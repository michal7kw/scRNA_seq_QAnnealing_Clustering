...
Load data directly from saved Seurat Objects pbmc3k
...
```{r, eval=FALSE}
hist(pbmc3k$nFeature_RNA,
breaks = 100,
main="number of genes detected distribution",
...
## GAPDH Gene histogram (pbmc3k)

```{r, eval=FALSE}
grep("GAPDH", rownames(pbmc3k), value = T)
GAPDH_count <- pbmc3k@assays$RNA@counts["GAPDH",]

hist(GAPDH_count,
breaks = 50,
main="GAPDH distribution",
xlab="number of genes detected",
ylab="cells count",
freq=TRUE,
xlim=range(0:100)
...
## GAPDH Gene histogram (GSM1599500, i.e. GSE65525)

[Droplet barcoding for single-cell transcriptomics applied to embryonic stem cells 2015]

```{r, eval=FALSE}
count_GSM1599500 <- read.csv("data/additional/GSM1599501_K562_pure_RNA.csv", row.names = 1)
...
```{r}
remove(count_GSM1599500_log, GAPDH_count, means, sds, snrs)
```
...
Load data directly from saved Seurat Objects pbmc10k
...
Create data copies (to test normalization methods) pbmc10k
...
for (k in c(5,15)){
  for (coff in c(0, 0.3, 0.6)){
    for (d in c(5, 15, 30)){
      ...
      nx$write_gexf(G, paste0("./graphs/graph_snn_n", n, "_k", k, "_coff", coff, "_d", d, ".gexf"))
    }
  }
}
```
...
for k in r.k_range:
  for coff in coff_range:
    for d in r.d_range:
      file_name = ''.join(["graph_snn_n", str(n), "_k", str(k), "_coff", str(coff), "_d", str(d), ".gexf"])
      G = nx.read_gexf(file_name)
      pos = nx.spring_layout(G)
      plt.cla()
      nx.draw_networkx_nodes(G, pos, node_size=10, nodelist=G.nodes)
      ...
      file_name = ''.join(["./plots/graph_snn_n", str(n), "_k", str(k), "_coff", str(coff), "_d", str(d), ".png"])
      plt.savefig(file_name, bbox_inches='tight')
```
...
library(reticulate)
virtualenv_create("scrna_proj")
# py_install(c("pandas","networkx","matplotlib"), envname = "scrna_proj")
use_virtualenv("scrna_proj")
```
