...
for (i in 0:(N-1)){
 pbmc3k_Classic_cluster[[i+1]] <- mean(distance(pbmc3k_QA[,which(pbmc3k_QA[["seurat_clusters"]] == i)]@assays$SCT@data %>% as.matrix %>% t %>% as.data.frame, method = "jaccard"))
}
```

```{r}
# ------- do kmeans clustering -------
km.out <- kmeans(pbmc3k_QA@assays$SCT@data %>% as.matrix %>% t %>% as.data.frame, centers=6, nstart=5)
...
```
```{r}
Kmeans_stats <- cluster.stats(distance(pbmc3k_QA@assays$SCT@data %>% as.matrix %>% t %>% as.data.frame, method = "jaccard"), pbmc3k_QA[["kmeans"]][,1])
```
```{r}
